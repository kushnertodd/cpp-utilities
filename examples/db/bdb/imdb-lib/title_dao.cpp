...
int Title_DAO::load(Bdb_db &title_db, const std::string &text_file, Bdb_Errors &errors, char delimiter) {
  return Imdb_DAO<Title_DTO, Title_DTO_key>::load(title_db, text_file, errors, delimiter);
}
...
void Title_DAO::lookup(Bdb_db &title_db, const std::string &title_id, Title_DTO &title_dto, Bdb_Errors &errors) {
  ...
  Imdb_DAO<Title_DTO, Title_DTO_key>::lookup(title_db, title_dto, title_DTO_key, errors);
  Ratings_DTO ratings_dto;
  if (!errors.has())
    Imdb_DAO<Ratings_DTO, Title_DTO_key>::lookup(title_db, ratings_dto, title_DTO_key, errors);
  if (!errors.has()) {
...
void Title_DAO::save(Bdb_db &title_db, Title_DTO &title_dto, Bdb_Errors &errors) {
  Imdb_DAO<Title_DTO, Title_DTO_key>::save(title_db, title_dto, errors);
}

void Title_DAO_list::select(Bdb_db &title_db, Title_DTO_list &title_dto_list, Bdb_Errors &errors) {
  Imdb_DAO_list<Title_DTO_key, Title_DTO_list>::select(title_db, title_dto_list, errors);
}
