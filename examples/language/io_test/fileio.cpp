...
File_IO::File_IO(std::string &filename_, Errors &errors) : filename(filename_) {
...
std::string File_IO_text_read::get_file(Errors &errors) {
...
void File_IO_text_write::open(const std::string &filename, Errors &errors) {
  ...
  out_file.open(file, std::ios::out);
  const size_t buflen = 200;
  char buf[buflen];
  if (out_file.bad()) {
    File_IO::strerror_safe(buf, buflen, errno);
...
void File_IO_text_write::put_line(const std::string &line, Errors &errors) {
}
